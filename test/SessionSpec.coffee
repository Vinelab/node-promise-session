...
Session = require('../lib/Session').klass
...
describe '', ->
    ...
    describe 'Session', ->
        ...
        beforeEach -> session = new Session({session: prefix: 'test-session'}, redis)
        ...
        it 'generates the store key based on the namespace, prefix and id', ->
            expect(session.storeKey('mikey')).toBe("test-session:#{session.id}:mikey")

        it 'generates unique IDs for each session to avoid key-value pair clashes', ->
            first_session = new Session({session: prefix: 'test-session'}, redis)
            second_session = new Session({session: prefix: 'test-session'}, redis)
            third_session = new Session({session: prefix: 'test-session'}, redis)
